...
class ParticleTransformerWrapper(torch.nn.Module):
    def __init__(self, **kwargs) -> None:
        ...
        if self.use_residual_attn:
            ...
            self.residual_attn_norm = torch.nn.LayerNorm(embed_dim)
            self.residual_attn_weights = torch.nn.Parameter(torch.zeros(num_layers, embed_dim), requires_grad=True)
    ...
    def forward(self, points, features, lorentz_vectors, mask):
        if not self.use_residual_attn:
            return self.mod(features, v=lorentz_vectors, mask=mask)
        return self._forward_with_residual_attn(features, v=lorentz_vectors, mask=mask)
    ...
    def _forward_with_residual_attn(self, x, v=None, mask=None, uu=None, uu_idx=None):
        ...
        with torch.cuda.amp.autocast(enabled=self.mod.use_amp):
            ...
            x_cls = self.mod.norm(cls_tokens).squeeze(0)
